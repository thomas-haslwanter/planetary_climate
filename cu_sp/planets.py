...
planet_props.index = planet_props['name']

# Adjust to SI-units
planet_props.mass *= 5.9722e24  # mass of earth [kg]
planet_props.day *= 3600        # convert from [h] to [sec]
planet_props.year *= 24*3600    # convert from [earth-days] to [sec]

# Create units dictionary
units = ['none', 'm', 'm/s**2', 'fraction', 'W/m**2', 'kg', 'm', 'sec', 'None', 'sec', 'deg', 'deg', 'K', 'K', 'K', 'none', 'none']
unit_dict = dict(zip(planet_props.columns, units))
...
if __name__ == '__main__':
    ...
    print(planet_props.columns)    
    
    # Show the properties of Earth
    print('The Properties of Earth: -------')
    print(planet_props.loc['Earth'])    
    
    # Show the "mass"-s:
    print('Mass: ---------------')
    print(planet_props.mass)        
        
    # Print all the "Critical*" Parameters    
    print('"T" Parameters: ---------------------')
    print(planet_props.filter(regex='T.*'))    
